...
def test_main_generates_from_cli_text_into_current_directory(tmp_path, monkeypatch, capsys) -> None:
    ...
    async def fake_generate_audio(request, **_: object) -> GenerationResult:
        assert request.source_text == "Bonjour depuis la CLI"
        assert request.output_dir == tmp_path
        assert request.language_code == "en"
        output_path = tmp_path / "bonjour-depuis-la-cli-20260528-120000.wav"
    ...
    assert exit_code == 0
    assert str(tmp_path / "bonjour-depuis-la-cli-20260528-120000.wav") in stdout
...
def test_main_rejects_invalid_language_code(capsys) -> None:
    try:
        main(["--lang", "en-US", "Bonjour"])
    except SystemExit as exc:
        stderr = capsys.readouterr().err
        assert exc.code == 2
        assert "supporte par Supertonic" in stderr
        assert "na" in stderr
    else:
...
def test_main_help_lists_supported_languages(capsys) -> None:
    try:
        ...
        stdout = capsys.readouterr().out
        assert exc.code == 0
        assert "Langues supportees:" in stdout
...
def test_main_reads_stdin_when_available(tmp_path, monkeypatch, capsys) -> None:
    ...
    async def fake_generate_audio(request, **_: object) -> GenerationResult:
        assert request.source_text == "Bonjour via stdin"
        return GenerationResult(
...
def test_main_setup_warms_supertonic_and_translation(monkeypatch, capsys) -> None:
    ...
    assert exit_code == 0
    assert warmed == [True]
    assert "Modele Supertonic pret." in stdout
    assert "Modele de traduction pret." in stdout
...
def test_main_setup_can_skip_translation(monkeypatch, capsys) -> None:
    ...
    assert exit_code == 0
    assert warmed == [False]
    assert "Modele de traduction ignore." in stdout